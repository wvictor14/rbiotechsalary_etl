with
    source as (select * from {{ source("survey_results", "src_responses_2025") }}),

    renamed as (

        select
            {{ dbt_utils.generate_surrogate_key(["'2025'", "timestamp"]) }}
            as response_id,
            2025 as survey_year,
            ...
            -- compensation
            -- 2025 has both the corrected and the typo'd currency question
            coalesce(
